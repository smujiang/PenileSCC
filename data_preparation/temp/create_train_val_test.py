...
data_out_root_dir = "/projects/wangc/jun/data/Penile_SCC_Study/processed_annotations_train_val_test_split"
...
for case_id in case_id_list:
    ...
    for a in all_img_list:
        if "mask" not in a and "Thumbs.db" not in a:
            img_list.append(a)
    ...
    for img in train_imgs:
        ...
        if JPG2PNG:
            ...
            shutil.move(img_fn, img_fn_to)
        if not os.path.exists(img_mask_fn):
            print("Source file doesn't exist %s" % img_mask_fn)
        elif os.path.exists(img_mask_fn_to):
            print("Has been moved to target place %s" % img_mask_fn_to)
        else:
    ...
    for img in val_imgs:
        ...
        if not os.path.exists(img_mask_fn):
            ...
            print("Has been moved to target place")
        else:
            shutil.move(img_mask_fn, img_mask_fn_to)
